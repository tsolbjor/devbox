...
| `NODE_MAJOR_VERSION` | 2026-09-24 → `24` | Even majors go LTS each October; 22 reached EOL 2026-09-23 |

Everything else floats: winget and apt packages follow their feeds, and the GitHub-release installers (`ensure_stern`, `ensure_kubectx`, `ensure_kubelogin`, …) resolve `latest` at run time.

Bumping a pin only affects a **fresh** provision, because the `ensure_*` functions that consume them short-circuit on "command already present". That is deliberate — a setup rerun silently replacing your Node major would be worse than leaving it — but it means the two halves are reconciled differently:

- **dotnet** is the exception: SDKs install side by side, so `ensure_dotnet` keys on the *pinned* SDK rather than on `dotnet` existing at all. A plain `bash setup-ubuntu.sh` adds the new one and removes nothing.
- **node and kubectl** replace the major version, so they stay guarded. `bash update-ubuntu.sh --pins` reconciles them, asking before each.
