...
| `setup-ubuntu.sh` | Bash | Normal user | Installs apt packages, configures Git globally, generates SSH key, creates `~/code`. Shell stack: Starship is the *only* prompt engine; oh-my-zsh is installed as a framework only (`ZSH_THEME=""`, `OMZ_PLUGINS="git"`) with zsh-autosuggestions/zsh-syntax-highlighting coming from apt, so nothing is loaded twice |
