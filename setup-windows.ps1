...
function Ensure-WindowsOptionalFeatureEnabled {
  ...
  if ($feature.State -eq "Enabled") {
    Write-Host "✓ Feature enabled: $FeatureName"
    return $false
  }
...
function Ensure-WSL {
  ...
  Write-Host "→ Setting WSL default version to $DefaultVersion"
  ...
  if ($distros -contains $DistroName) {
    Write-Host "✓ Distro installed: $DistroName"
  } else {
...
function Ensure-WSLConfigFile {
  ...
  if ($current -ne $desiredText) {
    ...
    Write-Host "✓ Updated .wslconfig (run: wsl --shutdown to apply without reboot)."
  } else {
    Write-Host "✓ .wslconfig already matches desired settings."
  }
}
...
function Ensure-WindowsTerminalDefaultProfileUbuntu {
  ...
  if ($json.defaultProfile -eq $ubuntu.guid) {
    Write-Host "✓ Windows Terminal default profile already set to: $($ubuntu.name)"
    return
  }

  Write-Host "→ Setting Windows Terminal default profile to: $($ubuntu.name)"
  ...
  Write-Host "✓ Updated Windows Terminal default profile."
}
...
function Ensure-VSCodeExtensions {
  ...
  foreach ($ext in $Extensions) {
    if ($installed -contains $ext) {
      Write-Host "✓ VS Code extension installed: $ext"
    } else {
...
if ($Config.InstallRancherDesktop)  { Install-WingetPackage -Id "Rancher.RancherDesktop" }
...
Write-Host "`nDone."
Write-Host "Tip: Apply WSL resource changes with: wsl --shutdown"